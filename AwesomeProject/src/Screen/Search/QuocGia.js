...
useEffect(()=>{
const resAPI=async()=>{
    const kp= await requestApi('quocgia','GET')
    setData(kp.quocgia)
    const kp1 = await requestApi(`phims/quocgia/${id}`,'GET')
...
resAPI()
  ...
  const handleItemPress =async (item) => {
     try {
        const kp = await requestApi(`phims/quocgia/${item}`,'GET')
    ...
    <>
      ...
      >
        {data&&data.map((item,index)=>(
          ...
          onPress={() => handleItemPress(item.id_quocgia)}
          key={index}
        > 
          <Text style={styles.itemText}>{ item.name_quocgia}</Text>
        </TouchableOpacity>
        ))}
